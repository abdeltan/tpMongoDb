...
app.post('/api/auth/login', async (req, res) => {
    ...
    if (!user) {
        return res.status(400).json({
            status: 'erreur',
            error: 'Verifier mot de passe ou adresse email'
        });
    }
...
app.put('/api/user/edit', async function (req, res) {
    try {
        ...
        if (jwtstatus.status) {
            ...
            if (user) {
                user.prenom = req.body.firstname;
                user.nom = req.body.lastname;
                user.telephone = req.body.phone;
                user.dateNaissance = req.body.dateOfBirth;
                user.save();
...
function checkJWT(full_token) {
    ...
    if (!token)
        return {
            status: false,
            message: 'Auth token est pas fourni'
        };
    else {
        try {
